...
output_fname = os.path.join(dir_path, '02_time_averaging.csv')
...
def _mean_feature(records, header):
    # get average value in the same time slot
    features = {}
...
def _averaging(records):
    ...
    while(True):
        ...
        if len(bucket) > 0:
            ...
            info_feature['TIMESTAMP'] = start_time
            refined_record = {**info_feature}
            for key in VITAL_SIGNS_HEADER + LAB_HEADER:
